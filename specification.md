...
- **Форма заявки** (адаптация под практику, 2026-08-18):
  - **Объект исследования**: поле «ЕКН» — подсказки по частичному совпадению
    ...
    продукт, введите название»: заказчик указывает название/толщину/целевой
    показатель сам, при сохранении заявки они отправляются в справочник ЕКН
    (`saveManualEknProduct` → `sbe-ekn` → `POST /api/ekn/manual/{ekn}`,
    непроверенная в QRC запись — см. `server_back/ekn-service/AGENTS.md`) —
    следующая заявка с тем же ЕКН подставит название автоматически. Поля
    названия/толщины/целевого показателя общие для ЕКН и без ЕКН — не пропадают
    при вводе номера (только пропадает надпись «Экспериментальный образец
