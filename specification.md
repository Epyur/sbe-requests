...
см. lab-service/AGENTS.md; в sbe-requests своих привилегий у superadmin нет, кроме
собственно назначения/снятия роли superadmin другим). Просмотр — viewer; создание/правка
заявок, справочников (objects), проектов, групп, статусы — editor+; labs/methods и права —
admin+; создание лабораторий (`POST /labs`, используется в ЛИМС, не в этом плагине) —
только superadmin. Правка заявки — только владелец или admin+ (сервер). Участники групп —
владелец группы или admin+. Общий доступ `{app}_common_access` даёт роль по умолчанию
(viewer/editor).
